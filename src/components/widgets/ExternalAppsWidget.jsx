...
import { SiGoogledocs, SiFigma, SiWhatsapp, SiTelegram } from 'react-icons/si'
import { supabase } from '../../lib/supabase'

// Icon mapping for external apps
const EXTERNAL_APPS_ICON_MAP = {
...
// Default external apps shortcuts configuration
const DEFAULT_EXTERNAL_APPS_SHORTCUTS = [
  { name: 'Google Docs', url: 'https://docs.google.com', icon_key: 'googledocs' },
  { name: 'Figma', url: 'https://figma.com', icon_key: 'figma' },
  { name: 'WhatsApp', url: 'https://web.whatsapp.com', icon_key: 'whatsapp' },
  { name: 'Telegram', url: 'https://web.telegram.org', icon_key: 'telegram' },
]

export default function ExternalAppsWidget() {
  const [shortcuts, setShortcuts] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    loadExternalAppsShortcuts()
  ...
  const loadExternalAppsShortcuts = async () => {
    try {
      const {
        data: { user },
      ...
      // Load only external apps shortcuts (googledocs, figma, whatsapp, telegram)
      const { data, error } = await supabase
        .from('shortcuts')
        .select('*')
        .eq('user_id', user.id)
        .in('icon_key', ['googledocs', 'figma', 'whatsapp', 'telegram'])
        .order('display_order', { ascending: true })

      if (error) throw error

      const existingIconKeys = (data || []).map((s) => s.icon_key)
      const missingDefaults = DEFAULT_EXTERNAL_APPS_SHORTCUTS.filter(
        (defaultShortcut) => !existingIconKeys.includes(defaultShortcut.icon_key)
      )

      // If any default external apps shortcuts are missing, add them
      if (missingDefaults.length > 0) {
        ...
        setShortcuts(
          allShortcuts.map((s) => ({
            ...s,
            icon: EXTERNAL_APPS_ICON_MAP[s.icon_key] || SiGoogledocs,
          }))
        )
      } else if (data && data.length > 0) {
        // Use saved external apps shortcuts
        setShortcuts(
          data.map((s) => ({
            ...s,
            icon: EXTERNAL_APPS_ICON_MAP[s.icon_key] || SiGoogledocs,
        ...
        // Initialize with defaults if no external apps shortcuts exist
        const defaultShortcutsWithIcons = DEFAULT_EXTERNAL_APPS_SHORTCUTS.map((s) => ({
          ...s,
          icon: EXTERNAL_APPS_ICON_MAP[s.icon_key] || SiGoogledocs,
        }))
      ...
      console.error('Error loading external apps shortcuts:', error)
  ...
  const saveDefaults = async (userId) => {
    try {
      // Get current max display_order for shortcuts to append external apps shortcuts
      ...
      const shortcutsToInsert = DEFAULT_EXTERNAL_APPS_SHORTCUTS.map((s, index) => ({
        user_id: userId,
        name: s.name,
        url: s.url,
        icon_key: s.icon_key,
      ...
      console.error('Error saving default external apps shortcuts:', error)
    }
  }

  const addMissingDefaults = async (userId, missingDefaults, existingExternalAppsShortcuts) => {
    try {
      // Get the max display_order from existing external apps shortcuts, or from all shortcuts
      let maxOrder = 0
      if (existingExternalAppsShortcuts.length > 0) {
        maxOrder = Math.max(...existingExternalAppsShortcuts.map((s) => s.display_order || 0)) + 1
      } else {
      ...
      console.error('Error adding missing external apps shortcuts:', error)
    }
  }
  ...
  if (shortcuts.length === 0) {
    return (
      <div style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
        No external apps added. Add some in Settings.
      </div>
    )
  }
